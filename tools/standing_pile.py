...
Read-only: the store is opened as it stands and nothing is written to it,
the registries and journals are read as the loop reads them, and no session
is started. The close of card #68 ran it against Hello Revenue before and
after the fold, so the close-out names every lane by number, evidence,
...
from runtime import launch, machine, reasons  # noqa: E402
...
def main(argv: list[str]) -> int:
    ...
    for card in store.cards(slug):
        ...
        if stood == Disposition.CLOSED:
            ...
            verdict = "the board brings it back (or waits on an end it reads)"

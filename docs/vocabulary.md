...
- **dial** — the owner's standing ruling that a verified defect enters
  execution without him — a switch per board, a line per board, and one
  number of lanes for the machine. Say "defects fix themselves", "the
  auto-fix setting", "this board's auto-fix", "where auto-fix stops".

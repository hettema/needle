...
**Findings:** 5 — 2 demonstrated defects (both fixed), 3 hypotheses (all ruled on, no change)
...
**Completion:** both demonstrated findings are fixed inside the change, with a test each that fails on the old behaviour; the three hypotheses are answered with evidence and a written reason, two of them now stated in the code where the next session reads them. Nothing is left open.
...
`tests/api/test_hook_script.py`. Migration 0025 is numbered 0025/0024 in this lane and

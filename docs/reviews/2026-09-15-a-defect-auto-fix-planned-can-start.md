...
## On the board itself, after the fold

The one check the suite cannot make, taken on the real board once the fold landed and the laptop was rebuilt and restarted:

- **Item 2, live.** Hello Revenue #453 — the card this was filed for — had a fix lane reading `ended`, note "the planning session d9f29045 finished its turn without a plan or a question and was stopped", with a plan on its card since. At 20:51:34Z the beat brought it back: stage `planned`, `planned_at` stamped, note "its plan arrived after the board had called its session ended", and its history reads "The plan landed (docs/plans/2026-09-15-a-specialist-that-uses-tools-keeps-its-own-reasoning-between-the-calls.md) after the planning session was called ended". Its Start is held by a collision with #554's lane, which is another card's business and exactly what should hold it.
- **The bound, live.** Seven cards stood in that shape before the restart. After the first beat that acted, six remain: one a beat, as designed, rather than seven lanes opening at once on a board whose number is four.
- **Item 4, live.** `needle fixes all --stranded --count` runs on the board that serves it and printed 9. It is a board verb, so it could not be run from the rented machine until the fold; this is its first real reading and the Loop's starting number. It is not zero today because the backlog above is still draining and because cards genuinely wait on the owner; the Loop watches it reach zero by 2026-10-15.

...
**Met:** `api/loops.py::_tell_owner` runs after the machine moves on every pass and rings what the record owes: the newest machine move out of Executing with no `told` row after it, within an hour (`TELL_HORIZON_SECONDS`, so the first beat after the fold did not ring every old exit on three boards), and not within a minute of the owner's own Stop (`OWN_MOVE_SECONDS`). The popup carries the move's own detail. `tests/api/test_a_card_that_finishes_or_needs_you_rings.py`: a fold-and-close rings as the card enters Executed; a kill rings the journal's reason; the owner's Stop rings nothing though the machine writes the exit; the same exit reconciled twice rings once; a crash between the move and the ring rings at the next reconcile; a notifier that could not be reached is one row and no second attempt. `AuditKind.TOLD` reaches the page through the typed mirror and the record renders it as any row. The lane the machine brings back (#68, folded 2026-09-09) never leaves Executing — the recovery runs before the moves in the same pass, and a resumed lane reads working — so it rings nothing; a park is an exit and rings. Read from the pass order after the rebase, not held by a test of its own.
...
The WATCH row on the card names the same reading after twenty `told` rows; the readings above decide what the ring becomes. Needle's eleven minutes is the board he was already watching; Hello Revenue's three hours is the gap the ring is for.

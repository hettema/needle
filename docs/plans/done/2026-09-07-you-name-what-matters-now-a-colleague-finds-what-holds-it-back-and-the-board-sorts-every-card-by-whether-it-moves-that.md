...
**Status:** DONE — shipped 2026-09-09 by the lane on card #87; every item stanced below.
...
**Met:** `docs/FOCUS.md` is read on the corpus's beat (`infrastructure/corpus.py::read_focus` through `board/focus.py::parse_focus`, the head-field reader plans use) into `FocusDocument`, with every missing or unreadable line named in `doubts`; the ruling is one row in `focus_rulings` (migration 0015) and `board/focus.py::is_chosen` is the one place a ruling meets a document; the strip and `GET /api/projects/{slug}/focus` are one object from `Live.focus_of`, printed by `needle strip SLUG [--json]` — named `strip`, not `focus`, because `needle focus <ref>` already brings a session's window forward (`api/runtime_cli.py`); `needle strip --unbound --count` is the loop's trace. Held by `tests/board/test_focus.py` (the fixture reads whole, a missing measure and an unreadable recheck name their line, a ruling at A over a document at B is proposed), `tests/api/test_focus.py` (the verb and the API agree) and `tests/ratchets/test_the_lens_never_chooses_work.py` (a chosen strip over a mismatched fingerprint cannot be built).
...
and says which.
**Met:** the Focus door (`api/doors.py::Doors.focus`, `POST /api/projects/{slug}/focus`, `WindowKind.FOCUS`) opens a conversation in the project's checkout with `board/brief.py::focus_brief` — his words, the intent, the current focus, the sharpen-then-diagnose instruction, the one document it may write — and, with `moves`, the Hormozi walk that writes each move he names as a suggestion with the Found-by line; it refuses a second while one lives, naming it; the rail lists it as `Focus`; a document landing moves the strip to proposed and a conversation that ends without one leaves the strip saying so. Held by `tests/api/test_focus.py::test_the_focus_door_opens_a_conversation…` and `…a_suggestion_written_from_the_focus_conversation…`.
...
**Met:** the focus loop (`api/focus.py`, its own beat beside the dial's so the dial never imports a class) asks a colleague of the other make in a fresh thread (`runtime/launch.py::ask_codex`, `codex exec -s read-only -C <checkout>` with the `CheckAnswer` schema, never `resume`) once per proposal fingerprint, with the document, the sources its Evidence line names as the board read them, and the intent; the verdict lands through `Doors.focus_check` bound to the fingerprint; a re-edited document is read again; two calls that end without an answer leave the strip offering the ruling with the runtime's reason; disagreement is shown and never blocks the click. Held by `tests/api/test_focus.py::test_a_proposed_focus_gets_one_cold_check…` and `…test_a_check_that_ends_without_an_answer…`; the one-reader-per-kind ratchet passes.
...
the ratchet holds.
**Met:** the same loop reads every open card (`board/focus.py::READ_COLUMNS`) with no current reading, oldest first, one per beat, each in a thread of its own, with `leverage_brief` (the focus whole, the card's document whole, the rule that judges what the plan would move); the result lands through `Doors.leverage` / `needle leverage`, which refuses any word outside the four classes and three likelihoods and a likelihood off the first class, moves nothing, and binds to both fingerprints; a reading is stale when either changes and only the changed card is read again; a renamed focus stales all; the hold shows beside the class. `tests/ratchets/test_the_lens_never_chooses_work.py` refuses the import under the dial. Held by `tests/api/test_focus.py::test_every_open_card_is_read_once…` and `…test_the_verb_refuses_any_other_word…`.
...
**Deviated:** the comp (`docs/design/2026-09-09-the-focus-strip-and-the-leverage-lens.html`, Harbourmaster data through the real derivation) was written before the build and is not signed — the lane ran unattended; its two owner's calls are listed at its foot and in the review record. Everything else is met: the `Leverage` lens (`frontend/src/board/dnd.ts::leverageColumns`, `Board.tsx`) shows the board as `board/leverage.py::arrange` — one pure function over the cards, their classes and the columns' grammar — orders it, across every column, storing nothing; a test per rule in `tests/board/test_leverage.py`; the strip (`FocusBar`) in the plan's words and states; "Accept this order" with the count and each move unticked at will, applied through `Live.move` with the owner as the mover and the batch and class on each history, the batch as one `leverage_batches` row carrying every place before the move; "Put it back" restores every one and a hand move retires it; an unticked move is recorded and not re-proposed until the focus, the document or the class changes; the Start click carries the lens and the class on its history; `vitest` covers the view storing nothing, the unavailable state, the ticks and the lens on Start; `tsc` passes. Two additions the plan did not spell: a parked plan that helps remove the limit comes forward and a parked defect returns to the rail (his "pulls plans or suggestions/bugs forward"), and the wake is written on the move's history line rather than as a row.
...
**Met:** the click reads both measures once as the baseline (`focus_measures`) and refuses a document whose measure cannot be read, naming the line (the parse already names it and the click is not offered); the machine-readable measures are read on their own cadence by the loop; on the recheck's date a reading of the other make lands one word through `Doors.focus_recheck` — `diagnosis challenged`, `work not linked`, `expired`, or `holds`, the fourth added so a recheck can say nothing broke — and each of the three failure words shows its sentence and pauses the order (`board/focus.py::paused_why`), as does a missed recheck date and a changed outcome sentence; a paused order never orders. Held by `tests/api/test_focus.py::test_each_recheck_outcome…` and `…test_a_recheck_that_holds…`; the first real focus's rows are read by hand in the review record.
...
Loop: a focus shown as chosen with no ruling bound to its fingerprint — command uv --project /home/dennis/Work/needle run needle strip --unbound --count expect 0 by 2026-10-05 every 1d

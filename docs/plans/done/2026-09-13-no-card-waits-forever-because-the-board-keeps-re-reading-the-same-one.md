...
**Challenged:** 2026-09-14, by claude 893e3e9c (call 133): 3 material corrections before build — (1) the fuse counts readings by the text each was *opened* on, bound to the reading's record at its open (one nullable column, migration 0027), not a sum of landed-on-this-text plus all-time deaths: the sum would have bound parked and title readings to the wrong fingerprint and would have broken the promise below that a changed text is read once more; (2) routing carries the branch that chose it (`Reason`), so the guard's test is keyed to every branch of `routing_of` rather than to the five states, which five branches share; (3) the Loop above keys on card *and* text (`needle decisions` now prints the text's fingerprint), since a card whose text changed twice, or a parked card answered twice, is legitimately read three times.
...
**The evidence the board keeps.** `store.latest_triages(slug)` carries each card's latest reading with `document_fingerprint` and `source_fingerprint`; `store.windowless_sessions(slug, work=SessionWork.TRIAGE)` every reading opened, with `started_at` and `ended_at`; `store.triages(slug, number)` and `store.title_readings(slug, number)` the ones that landed. Everything the guard needs is already there. The fuse needs one thing more, found at the challenge round (2026-09-14): a reading that dies leaves no row saying which text it read, and the three readers bind to three different texts (a mark's reading to the document and its source, a title's to the title and essence, a parked card's to the document and every row), so the text is bound to the reading's own record when it opens — `windowless_sessions.text_fingerprint`, migration 0027 — and the count is one function over that column.
...
**Met:** the guard is `board/dial.py::seat_opens`, stanced per branch of `routing_of` through `Reason`; `tests/board/test_dial.py::test_the_seat_has_a_stance_on_every_branch_of_routing_and_opens_only_on_unverified_text` builds every branch through `routing_of` itself and fails on a branch without a stance; the three commit-bound sentences say when it was read, what it landed and that a commit moves it, on the card, the rail and `needle fixes` (`tests/api/test_the_triage_seat.py::test_a_card_whose_reading_cannot_move_it_is_never_read_again`). The hand-out was not dispatched: the search was done by this session, since every reader had to be read whole to change it, and the review note names them file by file.

### 2. A fuse no future hole gets past

Behind the guard, a cap that counts every reading opened on a card since its document's text last changed — landed or not — and past which the board opens no more on that text, and the card says so: read N times on this text, the board stopped, and what would start it again (a change to the document or its source). The existing `TRIAGE_ATTEMPTS` and `_readings_that_died` are the place to start; whether the fuse widens that counter or stands beside it is the lane's call, with one rule: the one legitimate re-read of a changed text (a `stale` card) always gets its reading, because the fuse keys on the text and a changed text is a new count. The lane's call, after the challenge round: the counter is widened, not doubled — one count of readings opened on the text as this card's reader reads it today, bound at each reading's open, replacing the count of deaths (`board/dial.py::readings_spent`, read by every reader of the seat and by the face through `Live.readings_spent`).
...
**Met:** `board/dial.py::readings_spent` counts every ended reading bound to the text the card's reader reads today (migration 0027 binds it at the open), `TRIAGE_ATTEMPTS` caps it, and every reader of the seat refuses past the cap before any other rule; the face says "stopped reading" (broken) with `stopped_words`, the head counts it under "cards the board stopped reading", `needle fixes` prints the sentence in place of the routing's; `tests/api/test_the_triage_seat.py::test_three_readings_on_one_text_stop_the_board_loudly_and_a_changed_text_starts_it_again` and `::test_the_fuse_holds_a_title_and_a_parked_card_alike_and_a_new_park_or_title_starts_it_again` drive a defect, a title and a parked card through it and past it.

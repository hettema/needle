...
**Status:** SHIPPED — built, reviewed (three passes and five cold reads by Codex; thirteen findings, eleven fixed in the lane, two outside the change filed) and folded on 2026-09-11 by the lane on card #124 (Claude Fable 5.1, then Claude Opus 5 after a usage-limit handoff); review record `docs/reviews/2026-09-11-a-sessions-message-is-answered-at-once-and-never-sent-twice.md`. Placed at the top of Up next on 2026-09-10 at the owner's word ("What do we need to fix to make the board run properly? Can you put those cards at the top of up next").

...
colleagues, and no other human in the loop. It was worked out over five months on a real product
...
colleague — and the person can see which without reading code.
...
Needle's profile may flatten the project's own legitimate truth.
...
rule, and says which capability it lacked.
...
judgment and is not handed out. A delegated result
...
reason to quietly stop handing work out.

## 4. Only what is written survives

A session's memory does not decay; it vanishes. There is no colleague to ask
and no thread to scroll. So: state why, not what — the what is in the work; name the rejected alternative when its rejection is
load-bearing; if it is not written, it did not happen. Every change says what
prompted it. Every plan carries its rulings with the
...
rule; it applies to systems, settings and working arrangements alike. The discriminator is how failure shows up. Silent or late — a
...
cannot inspect the work directly to find out. A partial result reported as whole is not unfinished, it is bad
...
strong observation is the one Z predicts and the rival explanations do not. A change whose loop never closed is a belief, and is reported as
...
being asked. A loop that depends on the person remembering is not closed. The cadence belongs to the intent, not the calendar: a build, a
...
expensive case. If a step leaves no trace, the loop begins by creating
one.
...
with it. Count traces, and expect the first metric to be wrong.
...
status. On Needle, the writing is the corpus below.
...
planned, not remembered when it is done. Plans align on
...
in its own instructions, and why.
...
Work is handed to a role, and a role's answer is checked. Hand out
...
ratchet. A tool that matches on process or command text can match
...
part of the picture the other cannot see — he holds the intent; the colleague
holds the work. Sometimes that means a summary; often
...
encoding earns its cost, markdown he opens locally when it does not.
...
tidied away. What Needle mechanises of this

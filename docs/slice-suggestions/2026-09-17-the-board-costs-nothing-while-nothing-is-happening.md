...
**Split into:** docs/slice-suggestions/2026-09-17-the-board-stops-asking-about-sessions-that-ended-days-ago.md
...
The board should cost nothing while nothing is happening, and today it asks every machine every thirty seconds, day and night, whether anyone is looking or not. Hello Revenue's card #249 shipped a board that heard its inputs instead of asking them, so that it "is idle when the world is idle"; Needle replaced that board on 2026-09-04 and the replacement's loop chooses, in writing, to notice a session that dies silently within thirty seconds, and in the same module promises that "a quiet machine costs nothing". The two written claims conflict, and neither `docs/INTENT.md`, `docs/HOW-WE-HOLD-IT.md` nor card #83 (whose ruling of 2026-09-11 sends the board to the rented machine for good after #123 and says nothing of idle cost) picks between thirty-second notice of a silent death and a board quiet at rest, or says whether the cost still matters once the board is rented. While this stands the owner pays a warm laptop and a shorter battery for a board nobody is looking at overnight, and once the board moves to the rented machine he pays a rented core for the same silence.

The repeated ssh questions about sessions that ended days ago, which most of the measured children below belong to, are the settled half of this document and live in the suggestion named under the title; what remains here is the thirty-second pass itself.
...
- In one 60-second sample the board had 46 distinct children: 41 `ssh rented -- needle cause <id> …` calls, two `git fetch`, two notification prompts. The 41 cause calls are the settled half, split out; the passes, the fetches and the prompts are what this half is about, and what the passes cost with the cause calls gone is not yet measured.
...
- `docs/slice-suggestions/2026-09-17-the-board-stops-asking-about-sessions-that-ended-days-ago.md` — the settled half of this document: the repeated questions the written hour already excludes.

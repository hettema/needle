...
function findCard(board: BoardState, number: number): CardSummary | null {
  for (const column of board.columns)
    for (const group of column.groups)
      for (const card of group.cards) if (card.number === number) return card;
  return null;
}

function cardBoxes(
  groupEl: Element,
): { number: number; top: number; height: number }[] {
  return Array.from(groupEl.querySelectorAll<HTMLElement>("[data-card]")).map(
    (el) => {
      const rect = el.getBoundingClientRect();
      return {
        number: Number(el.dataset["card"]),
        top: rect.top,
        height: rect.height,
      };
    },
...
export function Board({
  ...
  const [furled, setFurled] = useState<Set<Column>>(
    () => new Set<Column>(isWide() ? [] : ["Executed", "Done", "Not now"]),
  );
  ...
  const openColumns = useMemo<Column[]>(
    () =>
      board
        ? board.columns
            .map((c) => c.definition.column)
            .filter((c) => !furled.has(c))
        : [],
    [board, furled],
  );

  const unfurlMore = useCallback(
    (column: Column) => setUnfurledMore((s) => new Set(s).add(column)),
    [],
  );
  ...
  const controller = useMemo<LiftController>(() => {
    const retarget = (target: Place) => {
      ...
      if (board) {
        const column = board.columns.find(
          (c) => c.definition.column === target.column,
        );
    ...
    return {
      ...
      drop: () => {
        ...
        setLift(null);
        if (!current) return;
        if (!samePlace(current.target, current.from))
          void store.move(current.number, current.target);
      },
      cancel: () => setLift(null),
    };
  ...
  const onDragStart = (e: DragStartEvent) => {
    const data = e.active.data.current as
      { number: number; place: Place } | undefined;
    if (!data) return;
  ...
  const onDragMove = (e: DragMoveEvent) => {
    ...
    const data = e.over.data.current as
      { column: Column; group: string | null } | undefined;
    if (!data) return;
    const groupEl = document.querySelector<HTMLElement>(
      `[data-group="${CSS.escape(String(e.over.id))}"]`,
    );
    if (!groupEl) return;
    controller.retarget(
      targetInGroup(
  ...
  const moveTo = useCallback(
    async (number: number, column: string): Promise<boolean> =>
      store.move(number, {
  ...
  const readSignal = useCallback(
    async (number: number, delivered: boolean) => {
      ...
      try {
        ...
        setReadSaid(
          `#${number}: the reading did not land — ${e instanceof Error ? e.message : String(e)}`,
        );
      } finally {
        setReading(null);
        await store.refresh();
  ...
  const openAnIdea = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        setIdeaSaid(
          `Idea did not open: ${e instanceof Error ? e.message : String(e)}`,
        );
      } finally {
  ...
  const turnTheDial = useCallback(
    async (on: boolean, lanes: number) => {
      ...
      try {
        ...
        setDialSaid(
          `auto-fix ${state.dial.on ? "on" : "off"}, ${state.dial.lanes} fix lane${state.dial.lanes === 1 ? "" : "s"} at most`,
        );
      } catch (e) {
        setDialSaid(
          `The dial did not turn: ${e instanceof Error ? e.message : String(e)}`,
        );
      } finally {
  ...
  const planTogether = useCallback(async () => {
    ...
    try {
      ...
      setPlanSaid(
        `Plan did not open: ${e instanceof Error ? e.message : String(e)}`,
      );
    } finally {
      setPlanning(false);
    }
  ...
  const talkFocus = useCallback(
    (text: string) =>
      void throughFocus(() => openFocus(slug, text), "Focus did not open"),
    [slug, throughFocus],
  );
  const useThisFocus = useCallback(
    () =>
      void throughFocus(() => chooseFocus(slug), "The focus was not chosen"),
    [slug, throughFocus],
  );
  const proposeTheMoves = useCallback(
    () =>
      void throughFocus(() => proposeMoves(slug), "Propose moves did not open"),
    [slug, throughFocus],
  );
  const putItBack = useCallback(
    () => void throughFocus(() => putBack(slug), "Put it back did not land"),
    [slug, throughFocus],
  );
  ...
  const acceptTheOrder = useCallback(async () => {
    ...
    const numbers = proposed
      .filter((n) => !unticked.has(n))
      .sort((a, b) => a - b);
    setFocusBusy(true);
    ...
    try {
      ...
      setAcceptSaid(
        `Accept did not land: ${e instanceof Error ? e.message : String(e)}`,
      );
    } finally {
      setFocusBusy(false);
      await store.refresh();
  ...
  if (!board) {
    return (
      <>
        ...
        {store.error ? (
          <Notice>The board could not be read: {store.error}</Notice>
        ) : (
          <Notice quiet>Reading the board…</Notice>
        )}
      </>
    );
  }
  ...
  const listed = projects.some((p) => p.slug === slug)
    ? projects
    : [board.project, ...projects];
  const corpusState = board.corpus.watching ? (
    <Strong>watching</Strong>
  ) : (
    <Off>
      not watching
      {board.corpus.watch_note ? ` — ${board.corpus.watch_note}` : ""}
    </Off>
  );
  const streamState = store.connected ? null : (
    <Off>
      {" "}
      · page not connected, showing the board as read {ago(board.generated_at)}
    </Off>
  );

  const heard = board.watercooler.length
    ? (board.watercooler[board.watercooler.length - 1] ?? null)
    : null;
  ...
  const shown = admitted
    ? board.columns.map((column) => {
        const groups = column.groups
          .map((group) => ({
            ...group,
            cards: group.cards.filter((c) => keeps(c, admitted)),
          }))
          .filter((group) => group.cards.length > 0);
        return {
          ...column,
          groups,
          count: groups.reduce((n, g) => n + g.cards.length, 0),
        };
      })
    : board.columns;
  // Clicking the word that is already filtering clears it, narrowed or not:
  // the whole board is always one click away.
  const pick = (word: WordKey) =>
    setFilter((f) =>
      f !== null && f.word === word ? null : { word, claim: null },
    );
  const narrow = (word: WordKey, claim: Claim) => {
    ...
    setFilter((f) =>
      f !== null && f.claim === claim ? { word, claim: null } : { word, claim },
    );
  };
  ...
  const on = (claim: Claim) =>
    filter !== null &&
    (filter.claim === claim ||
      (filter.claim === null &&
        lines(attention, filter.word).some((l) => l.claim === claim)));
  const failed = Object.values(store.statuses).filter(
    (st) => st.kind === "failed",
  ).length;
  ...
  const ticked = new Set(
    board.leverage.moves.map((m) => m.number).filter((n) => !unticked.has(n)),
  );
    ...
    <ProjectContext.Provider value={{ slug, path: board.project.path, heard }}>
      <LiftContext.Provider value={controller}>
        <HeadFrame>
          <AppHead>
            ...
            <ProjectSwitcher
              ...
              facts={
                <>
                  <CorpusLine>
                    <Strong>{board.corpus.live_plans}</Strong> live plans ·{" "}
                    <Strong>{board.corpus.live_suggestions}</Strong> suggestions
                    · <Strong>{board.corpus.archived}</Strong> archived ·{" "}
                    {corpusState}
                    {streamState}
                  </CorpusLine>
                  <Fact>
                    <Strong>{attention.arrived_today}</Strong> arrived today ·{" "}
                    <Strong>{attention.unplanned_defects}</Strong> defects and{" "}
                    <Strong>{attention.unplanned_ideas}</Strong> ideas unplanned
                  </Fact>
                  {board.machine.missing.length ? (
                    <Fact meaning="broken">
                      the runtime cannot find:{" "}
                      {board.machine.missing.join(", ")}
                    </Fact>
                  ) : null}
                  ...
                  {board.trunk.level === null ? (
                    ...
                    <Fact meaning="proven">
                      trunk level with origin/develop
                    </Fact>
                  ) : (
                    <Fact meaning="broken">
                      trunk {board.trunk.behind} behind origin/develop
                    </Fact>
                  )}
                  {failed > 0 ? (
                    <Fact meaning="broken">
                      {failed} write{failed === 1 ? "" : "s"} failed — see the
                      card
                    </Fact>
                  ) : null}
                </>
            ...
            <Words>
              {WORDS.map((word) => (
                <Word
                  key={word.key}
                  label={word.label}
                  count={counted(attention, word.key)}
                  meaning={word.meaning}
                  on={filter?.word === word.key}
                  onClick={() => pick(word.key)}
                />
              ))}
            </Words>
            <HeadTools>
              <DialControl
                state={board.dial}
                onTurn={(on, lanes) => void turnTheDial(on, lanes)}
                disabled={dialTurning}
                said={dialSaid}
              />
              <Lens
                value={lens}
                options={LENSES}
                onChange={setLens}
                title="A lens, never a write: sorting changes what you see, never the rank. Drag needs Rank."
              />
              <IdeaDoor
                onOpen={(text) => void openAnIdea(text)}
                disabled={ideaOpening}
                said={ideaSaid}
              />
            </HeadTools>
          </AppHead>
          {filter ? (
            <Breakdown
              label={`What ${WORDS.find((w) => w.key === filter.word)?.label} counts`}
            ...
            >
              {lines(attention, filter.word).map((line) => (
                <Sub
                  ...
                  line={line}
                  meaning={filter.word}
                  on={filter.claim === line.claim}
                  onClick={() => narrow(filter.word, line.claim)}
                />
              ))}
            </Breakdown>
          ) : null}
        ...
        <FocusBar
          strip={board.focus}
          onTalk={talkFocus}
          onChoose={useThisFocus}
          onPropose={proposeTheMoves}
          onPutBack={putItBack}
          disabled={focusBusy}
          said={focusSaid}
        />
        ...
        {on("signal asking") && board.asks.length ? (
          <AskList
            title={`${board.asks.length} shipped card${board.asks.length === 1 ? "" : "s"} wait${board.asks.length === 1 ? "s" : ""} on your reading — only you can read these signals, or a session read them and could not tell`}
          >
            {board.asks.map((a) => (
              <AskRow
                key={a.number}
                ...
                due={a.due}
                evidence={a.evidence}
                onRead={(delivered) => void readSignal(a.number, delivered)}
        ...
        {on("conversation") && board.conversations.length ? (
          <TalkList
            title={`${board.conversations.length} conversation${board.conversations.length === 1 ? "" : "s"} alive — never hands on a tree`}
          >
            {board.conversations.map((c) => (
              <TalkRow
                ...
                shortId={c.short_id}
                slot={c.slot}
                since={ago(c.started_at)}
              />
            ))}
          </TalkList>
        ) : null}
        {selected.size > 0 || planSaid ? (
          <TogetherBar
            count={selected.size}
            onPlan={() => void planTogether()}
            onClear={() => {
              setSelected(new Set());
              setPlanSaid(null);
            }}
        ...
        {store.error ? (
          <Notice>
            The board could not be re-read: {store.error}. Showing it as last
            read, {ago(board.generated_at)}.
          </Notice>
        ) : null}
        {board.trunk.note ? (
          <Notice>
            The main checkout is not level with origin/develop:{" "}
            {board.trunk.note}
          </Notice>
        ) : null}
        {on("document without card") && board.documents_without_card.length ? (
          <Notice>
            Documents in the corpus with no card — the watcher should have
            carded them; it did not:
            <List
              items={board.documents_without_card.map(
                (d) => `${d.path} — ${d.title}`,
              )}
            />
          </Notice>
          ...
          >
            ...
            <DragOverlay dropAnimation={null}>
              {lifted && lift?.by === "pointer" ? (
                <CardShell
                  number={lifted.number}
                  meaning={lifted.state.meaning}
                  label={`#${lifted.number} in flight`}
                  lift
                  isStatic
                >

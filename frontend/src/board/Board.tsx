...
function machineWords(m: MachineRoom): string {
  ...
  // A hook queue still holding an event after an hour is one the board never
  // answered (card 124), said on the machine's line; a count nobody took (an
  // older needle there, or a queue that could not be read) is said as not
  // read, never shown as nothing queued.
  ...
  const queued =
    unanswered === null || unanswered === undefined
      ...
      : unanswered
        ...
        : "";
  // A room that stands from an earlier pass (card 123): the machine has not
  // answered since, and the age says how old what the board shows is.
  const old = m.why && m.observed_at ? `as last read ${ago(m.observed_at)} ago — ${m.why}; ` : "";
  if (m.room.full) return `${old}${m.room.sentence ?? "full"}${clones}${queued}${behind}`;
  const gb = (m.room.available / 1024 ** 3).toFixed(1);
  return `${old}${gb} GB free${m.killed ? `, ${m.killed} killed today` : ""}${clones}${queued}${behind}`;
}
          ...
          facts={
            <>
              ...
              {(board.machine.machines ?? []).length > 1 ? (
                <Fact {...((board.machine.machines ?? []).some((m) => m.room === null || m.why || (m.room.stale_queue ?? 0) > 0) ? { meaning: "broken" as const } : {})}>
